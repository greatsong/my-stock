...
KPOP_STOCKS = {
    ...
    "SM엔터테인먼트 (041510.KQ)": "041510.KQ",
    "YG엔터테인먼트 (122870.KQ)": "122870.KQ",
    "JYP Ent. (035900.KQ)": "035900.KQ",
    "스타쉽엔터 (087600.KQ)": "087600.KQ",
    "큐브엔터 (182360.KQ)": "182360.KQ",
    "디어유 (376300.KQ)": "376300.KQ",
    "CJ ENM (035760.KS)": "035760.KS",
    "카카오 (035720.KS)": "035720.KS",
    "네이버 (035420.KS)": "035420.KS",
}

BENCHMARK = {"코스피 (KOSPI)": "^KS11"}
...
# ---------------- 데이터 로드 함수 ----------------
@st.cache_data(ttl=600, show_spinner="K-POP 관련주 데이터 불러오는 중...")
...
# ---------------- 사이드바 ----------------
st.sidebar.title("🎤 K-POP 분석 설정")

selected_names = st.sidebar.multiselect(
    "비교할 종목 선택 (최대 6개 권장)",
    list(KPOP_STOCKS.keys()),
    default=["하이브 (352820.KS)", "SM엔터테인먼트 (041510.KQ)", "YG엔터테인먼트 (122870.KQ)", "JYP Ent. (035900.KQ)"]
)

focus_name = st.sidebar.selectbox("상세 분석 종목", list(KPOP_STOCKS.keys()), index=0)
focus_ticker = KPOP_STOCKS[focus_name]
...
# ---------------- 헤더 ----------------
st.title("🎤 K-POP 관련주 전문 분석 대시보드")
st.caption("하이브, SM, YG, JYP 등 K-POP 엔터테인먼트 핵심 기업 심층 분석")
...
with tab1:
    ...
    try:
        ...
        if hist is None or hist.empty:
            ...
            col1.metric("현재가", f"{current:,.0f}원", f"{change_pct:+.2f}%")
            col2.metric("기간 최고", f"{high_period:,.0f}원")
            col3.metric("기간 최저", f"{low_period:,.0f}원")
            col4.metric("시가총액", f"{info.get('marketCap', 0)/1e12:,.2f}조원" if info.get('marketCap') else "N/A")
...
with tab2:
    ...
    if len(selected_names) < 1:
        ...
        try:
            tickers = [KPOP_STOCKS[n] for n in selected_names] + list(BENCHMARK.values())
            ...
            if data is None or data.empty:
                ...
                fig = go.Figure()
                all_names = selected_names + list(BENCHMARK.keys())
                all_tickers = tickers
                ...
                for name, tkr in zip(all_names, all_tickers):
                    try:
                        ...
                        if len(closes) > 0:
                            ...
                            is_benchmark = tkr in BENCHMARK.values()
                            fig.add_trace(go.Scatter(
                                x=normalized.index, y=normalized.values,
                                mode="lines", name=name,
                                line=dict(width=3 if is_benchmark else 2, dash="dash" if is_benchmark else "solid")
                            ))
                    except Exception:
                        continue
...
with tab3:
    ...
    compare_names = selected_names if selected_names else list(KPOP_STOCKS.keys())[:6]

    try:
        rows = []
        for name in compare_names:
            tkr = KPOP_STOCKS[name]
            info = load_single_info(tkr)
            rows.append({
                "종목": name,
                "현재가": info.get("currentPrice") or info.get("regularMarketPrice"),
                "시가총액(조원)": round(info.get("marketCap", 0) / 1e12, 2) if info.get("marketCap") else None,
        ...
        with col1:
            ...
            if not df_pe.empty:
                fig_pe = go.Figure(go.Bar(
                    ...
                    marker_color="#ec4899", text=df_pe["PER"], textposition="outside"
                ))
...
with tab4:
    ...
    if len(selected_names) < 2:
        ...
        try:
            tickers = [KPOP_STOCKS[n] for n in selected_names]
            data = load_batch(tickers)
